...
async def on_pay_overtime(message: Message) -> None:
    await send_rag_answer(
        message, question="Оплата сверхурочных и выходных", back_payload=CMD_PAY, category="pay",
    )
...
async def on_pay_bonus(message: Message) -> None:
    await send_rag_answer(
        message, question="Условия премирования", back_payload=CMD_PAY, category="pay",
    )
