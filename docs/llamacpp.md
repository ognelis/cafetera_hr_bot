...
## 3.6. Настройки Embedding-сервера (важно!)

Embedding-модель превращает текст в числовой вектор (набор чисел). Для этого она «читает» весь текст и в конце сжимает его в одну точку — это называется **pooling** (пулинг). Разные модели «смотрят» на разную часть текста:

| Модель | Нужный режим (`EMBED_POOLING`) | Что делает |
| --- | --- | --- |
| **Qwen3-Embedding** | `last` | Берёт последний токен — так модель обучена |
| nomic-embed-text | `mean` | Усредняет все токены |
| BGE / E5 | `cls` | Берёт первый специальный токен |

> **Аналогия:** представьте, что модель — это студент, который читает текст и записывает резюме. Pooling — это «где он ставит точку». Если сказать ему ставить точку не в том месте, резюме будет бессмысленным.

Вторая важная настройка — **EMBED_CTX_SIZE** (размер «рабочей памяти» сервера). При индексации документов сервер обрабатывает несколько текстов одновременно (пачками). Формула:

```
EMBED_CTX_SIZE >= EMBEDDING_CHUNK_SIZE × макс_токенов_на_чанк
```

По умолчанию: `8192 >= 16 × 512` — хватает с запасом. Если памяти не хватит, сервер упадёт с ошибкой SIGTRAP.

**Что делать:** ничего! Значения по умолчанию в `.env.example` уже настроены правильно для Qwen3-Embedding. Менять нужно только если вы переключаетесь на другую модель — тогда проверьте документацию модели на предмет нужного pooling-режима.

Соответствующие переменные в `.env`:

```
EMBED_POOLING=last           # режим пулинга (last/mean/cls)
EMBED_CTX_SIZE=8192          # размер KV-кэша embedding-сервера
EMBEDDING_CHUNK_SIZE=16      # сколько текстов в одной пачке
```
