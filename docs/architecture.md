...
11. [Оценка качества (RAGAS)](#11-оценка-качества-ragas)
...
## 11. Оценка качества (RAGAS)

Папка [`ragas/`](file:///Users/aleksandrrevenskij/Projects/cafetera_hr_bot/ragas) содержит независимый пайплайн оценки качества RAG.
...
Эвалюатор использует тот же `RagServiceSettings`, что и продакшен, поэтому оценивается реальная конфигурация. Подробнее — [docs/ragas.md](ragas.md).
